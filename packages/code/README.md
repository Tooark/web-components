...
- Properties: `value`, `variableKeys`, `completions`, `completionSource`, `formatter`, `canFormat`, `resolvedLineEnding`, `resolvedTheme`, `view` (the `EditorView`), and one per attribute; methods `format()`, `focus()`.
- Events: `change` (`detail: { value }`, user edits only), `ark-format-error` (`detail: { error }`).
- Keys: Ctrl/Cmd+F search, Ctrl/Cmd+Z/Y history, Ctrl+Space completions, Shift+Alt+F format, Tab/Shift+Tab indent, Esc+Tab leave.
...
- Types: `ArkCodeEditorOptions`, `ArkCodeLanguage`, `ArkCodeTheme`, `ArkCodeIndentStyle`, `ArkCodeLineEnding`, `ArkCodeCompletion`, `ArkCodeCompletionSource`, `ArkCodeFormatter`.
